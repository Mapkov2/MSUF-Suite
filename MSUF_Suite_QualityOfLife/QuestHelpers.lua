...
local function Gossip(self, event)
    ...
    if self.config.complete and type(C_GossipInfo.GetActiveQuests) == "function" then
        ...
        if S.Public(quests) and type(quests) == "table" then
            for i = 1, math.min(#quests, 100) do
                local q = quests[i]
                if S.Public(q) and type(q) == "table" and S.Public(q.isComplete) and q.isComplete and Allowed(self, q.questID) then
                    selected = q.questID
    ...
    if not selected and self.config.accept and type(C_GossipInfo.GetAvailableQuests) == "function" then
        ...
        if S.Public(quests) and type(quests) == "table" then
            for i = 1, math.min(#quests, 100) do
                local q = quests[i]
                if S.Public(q) and type(q) == "table" and Allowed(self, q.questID) then
                    selected = q.questID
...
function M:Refresh()
    ...
    if c.gossip and (c.accept or c.complete) then
        ctx:Event("GOSSIP_SHOW", Gossip)
        ctx:Event("GOSSIP_CLOSED", Gossip)
    else
        ctx:RemoveEvent("GOSSIP_SHOW")
        ctx:RemoveEvent("GOSSIP_CLOSED")
        self.gossipChosen = nil
    end
    ...
    if c.accept or c.complete or c.reward then
        ctx:Event("QUEST_FINISHED", Quest)
    else
        ctx:RemoveEvent("QUEST_FINISHED")
    end
end
