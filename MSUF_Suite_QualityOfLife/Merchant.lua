...
local function Repair(self)
    if not self.config.repair or type(CanMerchantRepair)~="function" or not CanMerchantRepair()
        or type(GetRepairAllCost)~="function" or type(RepairAllItems)~="function" or type(GetMoney)~="function" then return end
    ...
    if self.config.guildRepair then
        if type(CanGuildBankRepair)=="function" and type(GetGuildBankWithdrawMoney)=="function" and type(GetGuildBankMoney)=="function" then
            ...
            if Public(allowed) and allowed and Number(allowance) and Number(funds)
                and (allowance==-1 or allowance>=cost) and funds>=cost then RepairAllItems(true); return end
        end
...
local function SellJunk(self)
    ...
    if remaining then self.context:Event("BAG_UPDATE_DELAYED",function(module)
        for key in pairs(module.requested) do module.requested[key]=nil end
        SellJunk(module)
    end)
    else self.context:RemoveEvent("BAG_UPDATE_DELAYED") end
end
...
local function Merchant(self,event)
    if event=="MERCHANT_CLOSED" then
        ...
        self.context:RemoveEvent("BAG_UPDATE_DELAYED")
        if self.soldCount>0 and self.config.junkReport then NS.Print(string.format(S.Text("Sold %d junk items."),self.soldCount)) end
        self.soldCount=0
        return
    end
...
function M:Refresh()
    ...
    if c.repair or c.autoJunk then
        ctx:Event("MERCHANT_SHOW",Merchant);ctx:Event("MERCHANT_CLOSED",Merchant)
    else
        ctx:RemoveEvent("MERCHANT_SHOW");ctx:RemoveEvent("MERCHANT_CLOSED");ctx:RemoveEvent("BAG_UPDATE_DELAYED")
        self.merchantOpen=false
    end
    if not c.autoJunk then ctx:RemoveEvent("BAG_UPDATE_DELAYED") end
end
function M:Enable()
    self.merchantOpen=false;self.requested={};self.soldCount=0
    self:Refresh()
end
function M:Disable() self.merchantOpen=false;self.requested={};self.soldCount=0 end
S.Install("qol",M)
