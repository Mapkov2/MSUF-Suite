...
function D.StyleTimer()
    ...
    if not frame then
        ...
        frame:SetClampedToScreen(true); frame:SetFrameStrata("HIGH")
...
function D.UpdateTimer(liveDuration,liveResolved)
    ...
    if M.forced then seconds=M.lastDuration or SAMPLE_SECONDS
    elseif M.inCombat then
        if liveResolved then seconds=liveDuration else seconds=D.LiveDuration() end
    elseif c.timerKeep then seconds=M.lastDuration end
    if not seconds then
        if frame:IsShown() then frame:Hide() end
        return
    end
    seconds=floor(seconds)
    if seconds~=M.timerSecond then M.timerSecond=seconds; frame.text:SetText(D.Clock(seconds)) end
