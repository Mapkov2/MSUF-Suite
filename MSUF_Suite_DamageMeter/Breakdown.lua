...
local function StylePanel(win)
    ...
    panel.back:ClearAllPoints(); panel.back:SetSize(height,height); panel.back:SetPoint("TOPLEFT",panel,"TOPLEFT",0,0)
    D.FontStyle(panel.title,style.leftSize); panel.title:SetTextColor(style.leftR,style.leftG,style.leftB)
    panel.title:ClearAllPoints()
    panel.title:SetPoint("LEFT",panel.back,"RIGHT",3,style.baseline); panel.title:SetPoint("RIGHT",panel,"RIGHT",-3,style.baseline)
    D.FontStyle(panel.message,style.leftSize); panel.message:SetTextColor(.8,.8,.8)
    local top=-(height+style.spacing+6)
    panel.message:ClearAllPoints()
    panel.message:SetPoint("TOPLEFT",panel,"TOPLEFT",6,top+style.baseline); panel.message:SetPoint("TOPRIGHT",panel,"TOPRIGHT",-6,top+style.baseline)
...
function D.EnsurePanel(win)
    ...
    panel.title=S.CreateFontString(panel,nil,"OVERLAY"); panel.title:SetJustifyH("LEFT"); panel.title:SetWordWrap(false)
    panel.message=S.CreateFontString(panel,nil,"OVERLAY"); panel.message:SetJustifyH("CENTER")
    panel:Hide()
    return panel
end

local function SetMessage(panel,text)
    if text~=panel.messageText then panel.messageText=text; panel.message:SetText(text) end
end
...
function D.ShowPanel(win,blocked)
    ...
    win.statusText=""; win.status:SetText("")
    panel:Show()
end
...
function D.OpenBreakdown(win,index)
    ...
    if win.meterType==D.DEATHS then D.OpenRecap(win,source); return end
    local bd=win.bd
...
function D.OpenRecap(win,source)
    ...
    if Public(source.deathTimeSeconds) then open(id); return end
    local bd=win.bd
...
function D.CloseBreakdown(win,quiet)
    ...
    if tooltip and win.bdRows then
        ...
        for _,row in pairs(win.bdRows) do if owner==row then tooltip:Hide(); break end end
    end
...
function D.RenderBreakdown(win)
    ...
    for slot=1,capacity do
        ...
        if index<=count then
            if not row then row=D.CreateRow(panel,win,"spell"); rows[slot]=row; D.AnchorRow(row,panel,slot+1) end
            if groups then D.PaintGroup(row,groups[index],groups[1].amount,sum,duration,win.meterType)
            else D.PaintSpell(row,spells[index],source,win.meterType,bd.class) end
            row:Show()
...
function D.ScrollBreakdown(win,delta)
    ...
    if offset~=bd.offset then bd.offset=offset; D.RenderBreakdown(win) end
end
...
local function EnsureTip()
    ...
    tip:SetFrameStrata("TOOLTIP"); tip:SetClampedToScreen(true); tip:SetWidth(TIP_WIDTH)
    tip:Hide()
    local background=S.CreateTexture(tip,nil,"BACKGROUND")
    background:SetAllPoints(tip); background:SetColorTexture(.05,.05,.05,.92)
    tip.title=S.CreateFontString(tip,nil,"OVERLAY")
    tip.title:SetPoint("TOPLEFT",tip,"TOPLEFT",TIP_PAD,-TIP_PAD); tip.title:SetPoint("TOPRIGHT",tip,"TOPRIGHT",-TIP_PAD,-TIP_PAD)
    tip.title:SetJustifyH("LEFT"); tip.title:SetWordWrap(false)
    tip.message=S.CreateFontString(tip,nil,"OVERLAY")
    tip.message:SetPoint("TOPLEFT",tip.title,"BOTTOMLEFT",0,-4); tip.message:SetPoint("TOPRIGHT",tip.title,"BOTTOMRIGHT",0,-4)
...
local function TipContent(frame,win,source,session)
    ...
    for i=1,shown do
        ...
        if not row then
            row=D.CreateRow(frame,nil,"tip"); frame.rows[i]=row
            local y=-(TIP_PAD+18+(i-1)*TIP_ROW)
            row:SetPoint("TOPLEFT",frame,"TOPLEFT",TIP_PAD,y); row:SetPoint("TOPRIGHT",frame,"TOPRIGHT",-TIP_PAD,y)
        end
        if groups then D.PaintGroup(row,groups[i],groups[1].amount,sum,duration,win.meterType)
        else D.PaintSpell(row,spells[i],detail,win.meterType,class) end
        row:Show()
    end
...
function D.ShowTip(win,row)
    ...
    if frame.styleGen~=M.styleGen then
        ...
        D.FontStyle(frame.title,12); D.FontStyle(frame.message,11)
...
function D.RowClick(row,button)
    local win=row.win
    if button=="RightButton" then D.OpenTypeMenu(win,row); return end
    if row.index then D.HideTip(); D.OpenBreakdown(win,row.index) end
end
...
D.spellScripts={OnClick=D.BreakdownBack,OnEnter=D.SpellEnter,OnLeave=D.SpellLeave,OnMouseWheel=D.BreakdownWheel}
D.panelScripts={OnClick=D.BreakdownBack,OnMouseWheel=D.BreakdownWheel,OnEnter=D.HoverEnter,OnLeave=D.HoverLeave}
