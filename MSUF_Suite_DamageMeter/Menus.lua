...
local function TypePalette()
    ...
    return {
        ...
        idle = idle,
        hover = MixColor(idle, accent, .14),
        selected = MixColor(idle, accent, .30)
    }
end
local function TypeTileStyle(button, palette, selected, hovered)
    ...
    button.accent:SetColorTexture(palette.accent[1], palette.accent[2], palette.accent[3],
        selected and 1 or hovered and .85 or .48)
...
function D.OpenTypeMenu(win, owner)
    ...
    if panel:IsShown() and panel.win == win and panel.owner == owner then
        D.HideTypeMenu()
        return
    end
    ...
    if panel.styleGen ~= M.styleGen then
        ...
        for _, button in pairs(panel.buttons) do
            D.FontStyle(button.label, 11)
            D.FontStyle(button.arrow, 11)
        end
    end
