...
local function Points(frame)
    local points = {}
    for index = 1, math.min(frame:GetNumPoints(), 4) do
...
local function RecordGeometry(v, frame)
    ...
    if not record then
        record = {}
        v.geometry[frame] = record
    end
    return record
end

-- Anchors frame at one point (plus an optional second), remembering the
-- native points the first time we move it.
local function Place(v, frame, point, parent, relative, x, y, second)
    ...
    if not applied then
        applied = { {} }
        record.appliedPoints = applied
    end
    local first = applied[1]
    first[1], first[2], first[3], first[4], first[5] = point, parent, relative, x, y
    applied[2] = second
...
function Gear.SuppressListProvider(element, slotID)
    ...
    if not slots or not row or slots[slotID] ~= element
        or element ~= _G["Character" .. row.slotName] then
        return
    end
    Conceal(v, element.enchant)
    Conceal(v, element.trackLabel)
    Conceal(v, element.enchantWarningTooltip)
    if type(element.gems) == "table" then
        for index = 1, math.min(#element.gems, MAX_PROVIDER_GEMS) do
            Conceal(v, element.gems[index])
        end
    end
end
...
function Gear.RestoreLayout(v)
    ...
    if NS.IsCombatLocked() then
        NS.CombatGate.RunOrDefer("character-gear-layout", function()
            if not v.active or not v.host:IsVisible() then
...
local function PlaceListSlot(v, slot, row)
    if not slot or slot:GetWidth() <= 0 then return end
    local scale = 24 / slot:GetWidth()
    Scale(v, slot, scale)
    Place(v, slot, "LEFT", row.frame, "LEFT", 2 / scale, 0)
    row.icon:Hide()
    Gear.SuppressListProvider(slot, row.slot)
end

local function PlaceCosmeticSlot(v, slot, x)
    if not slot or slot:GetWidth() <= 0 then return end
    local scale = 24 / slot:GetWidth()
    Scale(v, slot, scale)
    Place(v, slot, "TOPLEFT", v.panel, "TOPLEFT", x / scale, -568 / scale)
end

local function ApplyListLayout(v)
    v.list, v.wide = true, false
    ...
    Conceal(v, v.root.InsetRight)
    Conceal(v, _G.CharacterStatsPane)
    Conceal(v, _G.CharacterModelScene)
    Conceal(v, _G.CharacterLevelText)
    Conceal(v, _G.PaperDollSidebarTabs)
    Conceal(v, _G.PaperDollFrame.TitleManagerPane)
    Conceal(v, _G.PaperDollFrame.EquipmentManagerPane)
    for _, row in ipairs(v.rows) do
        PlaceListSlot(v, _G["Character" .. row.slotName], row)
    end
    -- Cosmetic equipment remains accessible, outside the audited 16 rows.
    PlaceCosmeticSlot(v, _G.CharacterShirtSlot, 14)
    PlaceCosmeticSlot(v, _G.CharacterTabardSlot, 46)
end

local function ApplyWideLayout(v)
    v.wide, v.list = true, false
    ...
    for side, list in ipairs(columns) do
        for index, name in ipairs(list) do
            Place(v, _G["Character" .. name], "TOPLEFT", v.root, "TOPLEFT",
                side == 1 and 18 or 645, -90 - (index - 1) * 50)
        end
    end
...
function Gear.ApplyLayout(v)
    ...
    RepositionPanels(v.root, oldWidth, oldHeight)
end
...
local function Font(parent, size)
    ...
    font.gearSize = size
    font:SetFont(STANDARD_TEXT_FONT or DEFAULT_FONT, size, "")
...
function Gear.UpdateSummary(v)
    ...
    if not v.wideInfo then
        ...
        v.wideSummary = Font(host, 11)
        v.wideSummary:SetPoint("TOPLEFT", 18, -65)
        v.wideSummary:SetWidth(660)
        v.wideSummary:SetJustifyH("LEFT")
        v.wideFooter = Font(host, 10)
        v.wideFooter:SetPoint("BOTTOMLEFT", 18, 8)
        v.wideFooter:SetWidth(666)
        v.wideFooter:SetJustifyH("CENTER")
    end
    local path = FontPath()
    if v.wideInfoPath ~= path then
        v.wideSummary:SetFont(path, 11, "")
        v.wideFooter:SetFont(path, 10, "")
        v.wideInfoPath = path
    end
    v.wideSummary:SetText(v.summary:GetText() or "")
    v.wideFooter:SetText(v.footer:GetText() or "")
    v.wideSummary:SetTextColor(NS.Theme.GetColor("muted"))
    v.wideFooter:SetTextColor(NS.Theme.GetColor("muted"))
    v.wideInfo:Show()
end
...
-- True when the provider already shows non-empty text in this field.
local function ProviderText(element, key)
    ...
    if not NS.Safety.Public(text) then return false end
    return type(text) == "string" and text ~= ""
end

local function SetText(font, text)
    if font.gearText ~= text then
        font:SetText(text)
        font.gearText = text
    end
end

local function LayoutWide(a, row)
    local right = rightSide[row.slot] and row.slot ~= 16
    a.host:SetSize(222, 46)
    a.host:ClearAllPoints()
    a.host:SetPoint(right and "TOPRIGHT" or "TOPLEFT", a.slot, right and "TOPRIGHT" or "TOPLEFT",
        right and 4 or -4, 4)
    ...
    a.status:SetSize(222, 46)
    a.status:EnableMouse(true)
    NS.Surface.SkinOwnedButton(a.status, wideSpec)
    local x = right and 9 or 49
    a.name:ClearAllPoints()
    a.name:SetPoint("TOPLEFT", x, -5)
    a.name:SetSize(164, 14)
    a.enchant:ClearAllPoints()
    a.enchant:SetPoint("TOPLEFT", x + 14, -21)
    a.enchant:SetSize(150, 12)
    a.track:ClearAllPoints()
    a.track:SetPoint("TOPLEFT", x + 14, -32)
    a.track:SetSize(84, 10)
    a.enchantMark:ClearAllPoints()
    a.enchantMark:SetPoint("TOPLEFT", x, -21)
    a.enchantCheck:ClearAllPoints()
    a.enchantCheck:SetPoint("TOPLEFT", x, -21)
    a.upgradeMark:ClearAllPoints()
    a.upgradeMark:SetPoint("TOPLEFT", x, -32)
    a.upgradeCheck:ClearAllPoints()
    a.upgradeCheck:SetPoint("TOPLEFT", x, -32)
    a.upgradeBack:ClearAllPoints()
    a.upgradeBack:SetPoint("TOPLEFT", x + 14, -43)
    a.upgradeFill:ClearAllPoints()
    a.upgradeFill:SetPoint("TOPLEFT", x + 14, -43)
    a.qualityRail:ClearAllPoints()
    a.qualityRail:SetPoint(right and "TOPRIGHT" or "TOPLEFT", right and -1 or 1, -4)
    a.qualityRail:Show()
    a.upgradeBack:Show()
    a.enchant:SetJustifyH("LEFT")
    a.track:SetJustifyH("LEFT")
    for index, gem in ipairs(a.gems) do
        gem:SetSize(12, 12)
        gem:ClearAllPoints()
        gem:SetPoint("TOPLEFT", a.host, "TOPLEFT", x + 109 + (index - 1) * 14, -33)
    end
    a.name:Show()
end

local function LayoutCompact(a, row, legacy)
    a.name:Hide()
    a.qualityRail:Hide()
    a.enchantMark:Hide()
    a.enchantCheck:Hide()
    ...
    local offsetY = legacy and -1 or weapon and (14 - a.slot:GetHeight())
        or (row.slot == 1 or row.slot == 10) and 3 or -2
    a.host:SetPoint(right and "TOPRIGHT" or "TOPLEFT", a.slot, right and "TOPLEFT" or "TOPRIGHT",
        right and -6 or 6, offsetY)
    a.status:SetSize(width, 14)
    a.enchant:ClearAllPoints()
    a.track:ClearAllPoints()
    if legacy then
        a.enchant:SetPoint("TOPLEFT", 0, -17)
        a.enchant:SetSize(96, 14)
        a.track:SetPoint("TOPLEFT", right and 0 or 54, 0)
    ...
    for index, gem in ipairs(a.gems) do
        ...
        gem:SetPoint(right and "TOPRIGHT" or "TOPLEFT", a.host, right and "TOPRIGHT" or "TOPLEFT",
            (index - 1) * step * (right and -1 or 1), legacy and 0 or weapon and 12 or -17)
    end
...
local function ShowGemTooltip(gem)
    ...
    if gem.link then
        ...
        GameTooltip:SetText(gem.status == "empty" and NS.L.GEAR_SOCKET_EMPTY or NS.L.GEAR_GEM_UNKNOWN)
    end
    GameTooltip:Show()
end

local function PropagateClicks(frame)
...
local function Create(v, row, slot)
    ...
    status:SetScript("OnLeave", Leave)
    ...
    a.upgradeMark = Font(status, 11)
    a.enchantMark:SetSize(10, 12)
    a.upgradeMark:SetSize(10, 12)
    a.enchantCheck = status:CreateTexture(nil, "OVERLAY")
    a.enchantCheck:SetAtlas("checkmark-minimal")
    a.upgradeCheck = status:CreateTexture(nil, "OVERLAY")
    a.upgradeCheck:SetAtlas("checkmark-minimal")
    a.enchantCheck:SetDesaturated(true)
    a.upgradeCheck:SetDesaturated(true)
    a.enchantCheck:SetSize(10, 10)
    a.upgradeCheck:SetSize(10, 10)
    a.qualityRail = status:CreateTexture(nil, "ARTWORK")
    a.qualityRail:SetSize(2, 38)
    a.upgradeBack = status:CreateTexture(nil, "ARTWORK")
...
function Gear.Paint(v, row)
    ...
    local statusToken = a.statusToken or "muted"
    local upgradeToken = a.upgradeToken or "muted"
    a.enchant:SetTextColor(NS.Theme.GetColor(statusToken))
    a.track:SetTextColor(NS.Theme.GetColor(a.wide and a.upgradeToken
    ...
    if color then
        ...
        a.level:SetTextColor(NS.Theme.GetColor("text"))
        a.name:SetTextColor(NS.Theme.GetColor("text"))
        a.qualityRail:SetColorTexture(NS.Theme.GetColor("muted"))
    end
    a.enchantMark:SetTextColor(NS.Theme.GetColor(statusToken))
    a.enchantCheck:SetVertexColor(NS.Theme.GetColor(statusToken))
    a.upgradeMark:SetTextColor(NS.Theme.GetColor(upgradeToken))
    a.upgradeCheck:SetVertexColor(NS.Theme.GetColor(upgradeToken))
    a.upgradeFill:SetColorTexture(NS.Theme.GetColor(upgradeToken))
    local mutedR, mutedG, mutedB = NS.Theme.GetColor("muted")
    a.upgradeBack:SetColorTexture(mutedR, mutedG, mutedB, .2)
end

local function CompactStatus(info)
    if info.missingEnchant then return NS.L.GEAR_ENCHANT_MISSING_SHORT, "danger" end
    if info.emptySockets and info.emptySockets > 0 then
        return string.format(NS.L.GEAR_EMPTY_COMPACT, info.emptySockets), "danger"
    end
    if info.enchantRank then
        return string.format(NS.L.GEAR_ENCHANT_COMPACT, info.enchantRank, 2),
            info.lowEnchant and "warning" or "muted"
    end
...
local function WideState(a, info)
    ...
    if info.missingEnchant then
        mark, token, state = "!", "danger", "missing"
    elseif info.enchanted then
        mark = info.lowEnchant and "!" or ""
        token = info.lowEnchant and "warning" or "success"
        state = info.lowEnchant and "low" or "present"
    elseif info.enchantApplicable == false then
        mark, state = "-", "not-needed"
    end
    a.enchantState, a.statusToken = state, token
    SetText(a.enchantMark, mark)
    a.enchantMark:SetShown(state ~= "present")
    a.enchantCheck:SetShown(state == "present")
    ...
    local valid = type(current) == "number" and type(maximum) == "number"
        and current >= 0 and maximum > 0 and current <= maximum
    ...
    a.upgradeMark:SetShown(not complete)
    a.upgradeCheck:SetShown(complete)
    local ratio = valid and current / maximum or 0
    if ratio ~= a.upgradeRatio then
        a.upgradeFill:SetWidth(math.max(1, 84 * ratio))
        a.upgradeRatio = ratio
    end
...
local function WideStatus(a, info)
    local status = info.missingEnchant and NS.L.GEAR_ENCHANT_MISSING
        or info.enchanted and EnchantName(a, info)
        or info.enchantApplicable == false and NS.L.GEAR_ENCHANT_NOT_NEEDED
        or NS.L.GEAR_UNVERIFIED
    if a.enchantState == "low" then
        if a.lowName ~= status then
            a.lowName = status
            a.lowText = "1/2  " .. status
        end
        status = a.lowText
    end
    return status
...
local function UpdateGems(a, info, element)
    for index, gem in ipairs(a.gems) do
        local source = info.gemInfo and info.gemInfo[index]
        local external = element and element.gems and element.gems[index]
        local visible = info.sockets and index <= info.sockets and not (external and external:IsShown())
        ...
        if visible then
            gem.icon:SetTexture(source and source.texture or (gem.status == "empty"
                and "Interface\\ItemSocketingFrame\\UI-EmptySocket-Prismatic"
                or "Interface\\Icons\\INV_Misc_QuestionMark"))
            if gem.status == "empty" then
                gem.icon:SetVertexColor(NS.Theme.GetColor("danger"))
            else
...
function Gear.Update(v, row, slotName)
    ...
    local slots = ProviderSlots()
    local element = slots and slots[row.slot]
    ...
    local status, token = CompactStatus(info)
    a.statusToken = token
    local progress = info.upgradeText
        and (info.upgradeCurrent == info.upgradeMax and NS.L.GEAR_UPGRADE_MAX or info.upgradeText) or ""
    if wide then
        WideState(a, info)
        status = WideStatus(a, info)
        if a.upgradeState == "unknown" then
            progress = NS.L.GEAR_UPGRADE_UNKNOWN
        elseif info.upgradeTrack and progress ~= "" then
            progress = info.upgradeTrack .. "  " .. progress
        end
    ...
    a.enchant:SetShown(not externalEnchant)
    a.track:SetShown(not externalTrack)
    a.status:SetShown(wide or (not externalEnchant and status ~= "")
        or (not externalTrack and info.upgradeText ~= nil))
    if not a.status:IsShown() then Leave(a.status) end
    local showLevel = not ProviderText(element, "ilvl")
    a.level:SetShown(showLevel)
    a.levelBack:SetShown(showLevel)
    UpdateGems(a, info, element)
    Gear.Paint(v, row)
    a.host:Show()
end
