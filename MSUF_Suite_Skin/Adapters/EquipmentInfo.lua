...
-- Each rank-1 id is followed by its rank-2 id; every pair individually
-- verified in that snapshot.
local rank = {}
for _, id in ipairs({
    7934, 7936, 7938, 7956, 7958, 7960, 7962, 7964, 7966, 7968, 7970, 7972,
    ...
    8034, 8036, 8038, 8040, 8158, 8160, 8162, 8612, 8614, 8688,
}) do
    rank[id], rank[id + 1] = 1, 2
end
...
local statKeys = {
    "ITEM_MOD_CRIT_RATING_SHORT", "ITEM_MOD_HASTE_RATING_SHORT", "ITEM_MOD_MASTERY_RATING_SHORT",
    "ITEM_MOD_VERSATILITY", "ITEM_MOD_CR_LIFESTEAL_SHORT", "ITEM_MOD_CR_AVOIDANCE_SHORT",
    "ITEM_MOD_CR_SPEED_SHORT", "ITEM_MOD_STRENGTH_SHORT", "ITEM_MOD_AGILITY_SHORT",
    "ITEM_MOD_INTELLECT_SHORT", "ITEM_MOD_STAMINA_SHORT",
}
...
local function Field(object, key)
    object = Accessible(object)
    return type(object) == "table" and Accessible(object[key]) or nil
end
...
local function ItemIdentity(link, result)
    ...
    result.equip, result.texture = Text(equip), Number(texture)
    result.class, result.subclass, result.expansion = Number(class), Number(subclass), Number(expansion)
end

-- Clears a reused snapshot but keeps its nested tables.
local function Reset(result)
    ...
    for _, gem in ipairs(gems) do
        for key in pairs(gem) do gem[key] = nil end
    end
...
local function ReadGems(link, result)
    ...
    for index = 1, MAX_GEM_SLOTS do
        ...
        if not gem then
            gem = {}
            result.gemInfo[index] = gem
        end
        local id = C_Item.GetItemGemID(link, index)
        if not Public(id) then result.gemsRestricted = true end
        gem.id = Number(id)
...
local function ReadUpgrade(link, result)
    ...
    if result.upgradeCurrent and result.upgradeMax and result.upgradeMax > 0
        and result.upgradeCurrent <= result.upgradeMax then
        result.upgradeText = string.format("%d/%d", result.upgradeCurrent, result.upgradeMax)
    end
end

local function StripMarkup(text)
    return text and text:gsub("|A:.-|a", ""):gsub("|T.-|t", "")
...
local function ReadTooltip(unit, slot, result)
    ...
    for index = 1, math.min(#lines, MAX_TOOLTIP_LINES) do
        ...
        if kind and kind == types.ItemEnchantmentPermanent then
            result.enchantText = StripMarkup(Text(Field(line, "leftText")))
    ...
    if socketLines > 0 and not result.gems and not result.gemsRestricted then
        result.gems = filledLines
    end
    return lines
end

function Info.Invalidate(result)
    if result then result.settled = false end
end
...
function Info.Read(link, unit, slot, result, identity)
    ...
    Reset(result)
    result.link, result.identity = link, identity
    if not link then
        result.settled = true
        return result
    end
    ...
    local lines = ReadTooltip(unit, slot, result)
...
function Info.Check(result, level)
    ...
    if result.expansion == 11 and result.equip and result.class and result.subclass
        and level and level >= 90 then
        result.enchantApplicable = (result.class == 4 and armorEnchants[result.equip] == true
                and result.subclass <= 4
                and (result.equip ~= "INVTYPE_LEGS" or result.subclass >= 1))
            or (result.class == 2 and weapons[result.equip] == true
                and weaponClasses[result.subclass] == true)
        result.missingEnchant = result.enchantApplicable and result.enchantID == 0 or false
    end
    result.lowEnchant = result.enchantRank == 1
    result.unknown = result.pending or result.enchantID == nil or result.emptySockets == nil
        or result.enchantApplicable == nil or (result.enchanted and not result.enchantRank)
    result.warning = result.missingEnchant or result.lowEnchant
        or (result.emptySockets and result.emptySockets > 0)
    result.enchantLabel = result.missingEnchant and NS.L.GEAR_ENCHANT_MISSING or result.enchantText
        or (result.enchanted and NS.L.DOSSIER_ENCHANTED
            or result.enchantApplicable == nil and NS.L.GEAR_UNVERIFIED or "")
    result.enchantToken = result.missingEnchant and "danger" or result.lowEnchant and "warning"
        or result.enchanted and "success" or "muted"
    return result
end
...
function Info.AddTooltip(result)
    ...
    if result.enchantRank then
        GameTooltip:AddLine(string.format(NS.L.GEAR_ENCHANT_RANK, result.enchantRank, 2), 1, .82, 0)
    end
    if result.sockets and result.gems then
        GameTooltip:AddLine(string.format(NS.L.GEAR_SOCKET_DETAIL, result.gems, result.sockets,
            result.emptySockets or 0), .8, .8, .8, true)
    end
    if result.upgradeText then
        GameTooltip:AddLine((result.upgradeTrack or NS.L.GEAR_UPGRADE) .. " " .. result.upgradeText,
            .8, .8, .8, true)
    end
    ...
    for _, key in ipairs(statKeys) do
        ...
        if value and value > 0 and _G[key] then
            GameTooltip:AddLine(string.format("%s: %d", _G[key], value), .7, .8, .9)
        end
    end
    GameTooltip:AddLine(" ")
