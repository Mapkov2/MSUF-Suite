...
local firstDRPoint
function Stats.IsHost(frame) return hosts[frame] == true end
local definitions = {
...
local function Config() return NS.DB.characterStats end
local function Modern() return not NS.CharacterDetails or NS.CharacterDetails.IsModern() end
...
function Stats.StylesRows(pane)
    return pane and Config().enabled and Modern() and pane.ItemLevelCategory and pane.ItemLevelCategory.Title and Visible(pane)
end
...
local function FirstDRPoint(curve)
    ...
    local high = 200
    local value = Number(Read(curve, 21024, high))
    if not value or high - value < .001 then return nil end
    local low = 0
    for _ = 1, 18 do
        local middle = (low + high) / 2
        value = Number(Read(curve, 21024, middle))
...
function Stats.ReadRating(index, result)
    ...
    local baseline = Number(Read(curve, 21024, 1))
    if not baseline or math.abs(baseline - 1) > .001 then return result end
    local raw = rating * perPoint
    local effective = Number(Read(curve, 21024, raw))
    local nextPoint = Number(Read(curve, 21024, raw + perPoint))
    ...
    if not effective or not nextPoint or effective > raw + .001
        ...
        or nextPoint < effective or nextPoint - effective > perPoint * 1.002 then return result end
...
local function CaptureFont(font)
    ...
    local points = {}
    for i = 1, math.min(Number(Read(font.GetNumPoints, font)) or 0, 4) do
        points[i] = { font:GetPoint(i) }
    end
    local r, g, b, a = Read(font.GetTextColor, font)
    return { font = font, path = path, size = size, flags = flags or "", points = points,
        object = Read(font.GetFontObject, font), justify = Read(font.GetJustifyH, font), color = r and { r, g, b, a } or nil }
end
...
local function Font(v, font, size, token)
    ...
    if not saved then saved = CaptureFont(font);v.fonts[font] = saved end
    ...
    if token and saved.color then
        ...
        if saved.r ~= r or saved.g ~= g or saved.b ~= b or saved.a ~= a then
            font:SetTextColor(r, g, b, a);saved.r, saved.g, saved.b, saved.a = r, g, b, a
        end
    end
    return saved
end
local function Height(v, frame, height)
    ...
    local saved = v.heights[frame]
    if frame:GetHeight() ~= height then frame:SetHeight(height) end
    saved.applied = height
end
local function Token(result)
    return result.penalty and (result.penalty >= 20 and "danger" or result.penalty > 0 and "warning" or "muted") or "muted"
end
local function Paint(v)
    ...
    for _, record in pairs(v.rows) do
        ...
        if record.fontPath ~= v.path then
            record.meta:SetFont(v.path, 10, "");record.dr:SetFont(v.path, 9, "");record.fontPath = v.path
        end
    ...
    if v.helpPath ~= v.path then v.helpText:SetFont(v.path, 9, "");v.helpPath = v.path end
...
local function Help(v)
    ...
    for i, definition in ipairs(definitions) do
        ...
        if result and result.penalty then
            GameTooltip:AddLine(string.format(NS.L.STATS_DR_DETAIL, 100 - result.penalty, result.loss), .75, .8, .85, true)
            if result.inDR and result.lostRating then
                GameTooltip:AddLine(string.format(NS.L.STATS_DR_AMOUNT, result.inDR, result.lostRating), .75, .8, .85, true)
            end
        else GameTooltip:AddLine(NS.L.STATS_DR_UNKNOWN, .75, .8, .85, true) end
    end
    GameTooltip:AddLine(" ")
...
local function HideMetadata(v)
    for _, record in pairs(v.rows) do record.meta:Hide();record.dr:Hide();record.accent:Hide() end
    v.help:Hide()
    if GameTooltip and GameTooltip:IsOwned(v.help) then GameTooltip:Hide() end
end
local function Create(pane, owner)
    local v = { pane = pane, owner = owner, active = true, fonts = {}, heights = {}, rows = {}, ordered = {}, results = {},
        deferKey = "character-stats:" .. owner }
    for i = 1, #definitions do v.results[i] = {} end
    local host = CreateFrame("Frame", nil, pane);v.host = host;hosts[host] = true
    host:SetSize(1, 1);host:SetPoint("TOPLEFT");host:EnableMouse(false)
    local help = CreateFrame("Button", nil, host);v.help = help
    help:SetSize(28, 18);help:SetPoint("RIGHT", pane.EnhancementsCategory, "RIGHT", -5, 0)
    ...
    v.helpPath = FontPath();v.helpText:SetFont(v.helpPath, 9, "")
    v.helpText:SetAllPoints();v.helpText:SetText("DR")
    help:SetScript("OnEnter", function() Help(v) end)
    help:SetScript("OnLeave", function() if GameTooltip and GameTooltip:IsOwned(help) then GameTooltip:Hide() end end)
    host:SetScript("OnShow", function()
        if Enabled(v) then host:RegisterEvent("PLAYER_REGEN_DISABLED");Stats.Refresh(v) end
    end)
    host:SetScript("OnHide", function()
        host:UnregisterEvent("PLAYER_REGEN_DISABLED");NS.CombatGate.Cancel(v.deferKey);HideMetadata(v)
    end)
    host:SetScript("OnEvent", function()
        HideMetadata(v)
        NS.CombatGate.RunOrDefer(v.deferKey, function() if Enabled(v) and Visible(pane) then Stats.Refresh(v) end end)
    end)
    return v
end
local function Record(v, row)
    ...
    if not record then
        record = { frame = row, spec = { radius = 4, border = 0, inset = 1, allowImplicitProtected = true } };v.rows[row] = record
        record.meta = row:CreateFontString(nil, "OVERLAY")
        record.meta:SetPoint("TOPLEFT", 11, -22);record.meta:SetWidth(80);record.meta:SetJustifyH("LEFT")
        record.dr = row:CreateFontString(nil, "OVERLAY")
        record.dr:SetPoint("TOPRIGHT", -8, -22);record.dr:SetWidth(80);record.dr:SetJustifyH("RIGHT")
        record.meta:Hide();record.dr:Hide()
        record.accent = row:CreateTexture(nil, "ARTWORK")
        record.accent:SetSize(1, 12);record.accent:SetPoint("LEFT", 3, 0)
    end
    ...
    for i, definition in ipairs(definitions) do
        ...
        if type(text) == "string" and label == string.format(STAT_FORMAT or "%s:", text) then
            record.definition, record.result = definition, v.results[i];break
        end
    end
    return record
end
function Stats.Refresh(v)
    ...
    if NS.IsCombatLocked() then
        NS.CombatGate.RunOrDefer(v.deferKey, function() if Enabled(v) and Visible(v.pane) then Stats.Refresh(v) end end)
        return
    end
    ...
    for _, record in pairs(v.rows) do record.meta:Hide();record.dr:Hide();record.accent:Hide() end
    for _ = 1, 64 do
        local row = iterator(invariant, control);control = row
        if not row then break end
        if Visible(row) and row.Label and row.Value and NS.Safety.CanCreateRegions(row, true) then
            count = count + 1
            local record = Record(v, row);v.ordered[count] = record
            if record.definition then coreCount = coreCount + 1 end
        end
    end
    for i = #v.ordered, count + 1, -1 do v.ordered[i] = nil end
    if count == 0 then v.help:Hide();return end
    local wide=NS.GearAnnotations.IsWide()
    local categoryHeight,levelHeight,detailHeight=wide and 30 or 22,wide and 48 or 34,wide and 18 or 12
    local used = 8
    for _, field in ipairs(categories) do
        local category = v.pane[field]
        if category then Height(v, category, categoryHeight);if Visible(category) then used = used + categoryHeight end end
    end
    local itemLevel = v.pane.ItemLevelFrame
    if itemLevel then Height(v, itemLevel, levelHeight);if Visible(itemLevel) then used = used + levelHeight end end
    local room = math.max(0, v.pane:GetHeight() - used)
    local extra = Config().diminishingReturns and coreCount * detailHeight or 0
    ...
    local showDR = Config().diminishingReturns and detailsFit
    if not showDR then base = math.min(24, math.max(15, math.floor(room / count))) end
    if Config().diminishingReturns then
        for i, definition in ipairs(definitions) do Stats.ReadRating(_G[definition.rating], v.results[i]) end
    end
    Paint(v)
    ...
    v.headerSpec=v.headerSpec or {role="panel",listItem=true,border=0,radius=4,inset=1,allowImplicitProtected=true}
    v.ilvlSpec=v.ilvlSpec or {role="card",border=0,radius=4,inset=1,allowImplicitProtected=true}
    for _,field in ipairs(categories) do
        local category=v.pane[field]
        if category and v.fonts[category.Title] then
            category.Title:ClearAllPoints();category.Title:SetPoint("LEFT",category,"LEFT",11,0)
    ...
    for i = 1, count do
        local record = v.ordered[i]
        local row = record.frame
        local detailed = showDR and record.definition ~= nil
        ...
        for _, field in ipairs(fontFields) do
            ...
            if saved then
                if detailed then
                    ...
                    row[field]:SetPoint(field == "Label" and "TOPLEFT" or "TOPRIGHT", row,
                        field == "Label" and "TOPLEFT" or "TOPRIGHT", field == "Label" and 11 or -8, wide and -7 or -4)
                else RestorePoints(saved) end
            end
        end
        if detailed then
            record.meta:ClearAllPoints();record.meta:SetPoint("TOPLEFT",11,wide and -28 or -22)
            record.dr:ClearAllPoints();record.dr:SetPoint("TOPRIGHT",-8,wide and -28 or -22)
            local result = record.result
            local text=result.rating and string.format(NS.L.STATS_RATING,result.rating) or NS.L.STATS_RATING_UNKNOWN
            if record.metaText~=text then record.meta:SetText(text);record.metaText=text end
            local badge = Stats.DRBadge(result)
            if record.drText ~= badge then record.dr:SetText(badge);record.drText = badge end
            record.dr:Show()
            record.meta:Show()
        end
        record.accent:SetShown(detailed and record.result.penalty~=nil and record.result.penalty>0)
    end
    v.help:SetShown(Config().diminishingReturns and Visible(v.pane.EnhancementsCategory))
end
function Stats.Apply(pane, owner)
    if NS.IsCombatLocked() or not pane or not pane.ItemLevelCategory or not pane.ItemLevelCategory.Title
        or not NS.Safety.CanCreateRegions(pane, true) then return end
    local v = Stats.views[pane]
    if not Config().enabled or not Modern() then if v then Stats.Disable(pane, owner) end;return end
    if not v then v = Create(pane, owner);Stats.views[pane] = v end
    if v.owner ~= owner then return end
    v.active = true;v.host:Show();Stats.Refresh(v)
end
function Stats.Disable(pane, owner)
    ...
    v.active = false;NS.CombatGate.Cancel(v.deferKey);v.host:Hide();HideMetadata(v)
    v.host:UnregisterEvent("PLAYER_REGEN_DISABLED")
    for font, saved in pairs(v.fonts) do
        local r, g, b, a = Read(font.GetTextColor, font)
        if saved.object and font.SetFontObject then font:SetFontObject(saved.object)
        else font:SetFont(saved.path, saved.size, saved.flags) end
        if saved.tinted and saved.color then font:SetTextColor(unpack(saved.color))
        elseif r and g and b and a then font:SetTextColor(r, g, b, a) end
        RestorePoints(saved)
...
function Stats.RefreshFonts()
    ...
    for _, v in pairs(Stats.views) do if Enabled(v) and Visible(v.pane) then Paint(v) end end
end
function Stats.SetOption(key, value)
    if NS.IsCombatLocked() or (key ~= "enabled" and key ~= "diminishingReturns") or type(value) ~= "boolean" then return false end
    Config()[key] = value
