...
local function OnEnter(icon)
    ...
    if _G.GameTooltip_SetDefaultAnchor then _G.GameTooltip_SetDefaultAnchor(tip,icon) else tip:SetOwner(icon,"ANCHOR_RIGHT") end
    local slot=entry.equipSlot or (entry.src=="e" and entry.id)
    if slot then tip:SetInventoryItem("player",slot)
    elseif entry.src=="i" then tip:SetItemByID(entry.itemID or entry.id)
    else
        local spell=entry.tooltip or entry.spell or entry.base
        if not spell then tip:Hide();return end
...
function I.Formatter(seconds,r,g,b)
    ...
    if type(util)~="table" or type(util.CreateNumericRuleFormatter)~="function" then formatters[key]=false;return nil end
...
function I.Apply(entry)
    ...
    if icon.lastBling~=bling then icon.lastBling=bling;cooldown:SetDrawBling(bling) end
...
local function Pool(slotKey)
    ...
    if not pool then pool={byKey={},free={}};pools[slotKey]=pool end
    return pool
end
...
function I.Sync(slotKey)
    ...
    if not plan or plan.kind~=1 or not view then I.Release(slotKey);return end
    ...
    for key,icon in pairs(byKey) do
        if icon.mark~=syncGen then byKey[key]=nil;Recycle(pool,icon) end
    end
end
...
function I.Count(slotKey)
    ...
    if pool then for _ in pairs(pool.byKey) do count=count+1 end end
    return count
end
