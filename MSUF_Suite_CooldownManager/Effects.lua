...
local function EnsureGlow(icon)
    ...
    local edges={}
    for i=1,4 do edges[i]=S.CreateTexture(glow,nil,"OVERLAY");edges[i]:Hide() end
...
local function Paint(glow,icon,spec,style,r,g,b,w,h)
    ...
    if spec.atlas then
        ...
        if r then flip:SetVertexColor(r,g,b) else flip:SetVertexColor(1,1,1) end
        K.SetFlipBook(glow.flipAnim,spec)
        if glow.edgesOn then glow.edgesOn=false;for i=1,4 do edges[i]:Hide() end end
        if not glow.flipOn then glow.flipOn=true;flip:Show() end
    else
        local gold=K.GLOW_GOLD
        K.PlaceEdges(edges,icon,K.Pixels(spec.edge),r or gold[1],g or gold[2],b or gold[3],1)
        glow.edgesOn=true
        if glow.flipOn then glow.flipOn=false;flip:Hide() end
    end
end
...
local function HideGlow(icon)
    ...
    if anim then glow.anim=nil;anim:Stop() end
    glow:Hide()
end
...
local function Drop(spell)
    ...
    if count>0 then rangeRefs[spell]=count;return end
    rangeRefs[spell]=nil
