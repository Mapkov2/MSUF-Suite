...
local function GateComplete()
    for _,seen in pairs(gate) do if not seen then return false end end
    return true
end
...
local function Watch()
    ...
    if GateComplete() then SetReady();return end
...
function Catalog.Ready()
    ...
    if Public(ids) and type(ids)=="table" and #ids>0 then SetReady() else Watch() end
    return ready
end
...
local function Put(rec,field,value)
    if rec[field]~=value then rec[field]=value; changed=true end
end
local function CopyLinked(rec,list)
    ...
    if type(list)=="table" then
        for i=1,#list do
            local id=Num(list[i])
            if id then n=n+1; linkedTmp[n]=id end
        end
    end
    ...
    if old and #old==n then
        local same=true
        for i=1,n do if old[i]~=linkedTmp[i] then same=false; break end end
        if same then return end
    end
    if n==0 then rec.linked=EMPTY
    else
        local copy={}
...
local function Copy(id,info,category)
    ...
    if not rec then rec={id=id,key="b"..id}; records[id]=rec; changed=true end
...
local function ApplyLayout(layout)
    ...
    if type(saved)=="table" then
        ...
        for i=1,#saved do
            local id=saved[i]
            if id~=nil and fetched[id] and not kept[id] then kept[id]=true; m=m+1; merged[m]=id end
        end
        for i=1,n do
            local id=defaultOrder[i]
            if not kept[id] then m=m+1; merged[m]=id end
        end
...
local function Commit(dst,src)
    ...
    for i=1,n do
        if dst[i]~=src[i] then dst[i]=src[i]; diff=true end
    end
...
function Catalog.OnOverride(base,override)
    ...
    if recs then
        for i=1,#recs do
            local rec=recs[i]
            if rec.override~=override then rec.override=override; any=true end
        end
    end
    if list then
        for i=1,#list do
