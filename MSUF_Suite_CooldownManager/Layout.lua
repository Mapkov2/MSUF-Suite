...
local function Rect(unit)
    ...
    if watched[unit] ~= frame then
        if NS.IsCombatLocked() then
            ...
            watched[unit] = frame
            Probe(frame, "BOTTOMLEFT")
            Probe(frame, "TOPRIGHT")
        end
    end
...
local function PlaceAuras(bar, view, plan)
    ...
    local width, height
    if ordered then
        width, height = Fill(w, h, sp, per, vertical, grow, align, n1 + n2, 0, out, unit)
    else
