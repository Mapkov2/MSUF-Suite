...
local function Epoch()
    if profile~=NS.DB then profile=NS.DB;epoch=epoch+1 end
    return epoch
end
...
local function OwnerName(id) return "MSUFSuite."..id end
    ...
    for key in pairs(suppressed) do
        ...
        if type(record)=="table" and not wrapped[record] then
            ...
            record.isEnabled=function(...)
                if next(suppressed[key] or {}) then return false end
...
local function EnsureSession(api)
    ...
    elseif not anyListener and sessionAPI~=api and type(api.RegisterSessionListener)=="function" then
        api.RegisterSessionListener("MSUFSuite",SessionChanged);sessionAPI=api
    end
...
local function Place(frame,point,x,y)
    frame:ClearAllPoints();frame:SetPoint(point,UIParent,point,x,y)
end
local function Element(id,elementID,spec)
    ...
    local function Point() return type(spec.point)=="function" and spec.point() or spec.point or "CENTER" end
    return {
        id=elementID,label=S.Text(spec.label),group="MSUF Suite",order=spec.order or 1000,
        getFrame=Frame,
        isEnabled=function() return S.states[id].active and (not spec.isEnabled or spec.isEnabled()) and Frame()~=nil end,
        captureState=function()
            local c=S.Config(id)
            local values={[spec.xKey]=c[spec.xKey],[spec.yKey]=c[spec.yKey]}
            if spec.pointKey then values[spec.pointKey]=c[spec.pointKey] end
            for _, key in ipairs(spec.historyKeys or {}) do values[key]=c[key] end
            return {epoch=Epoch(),values=values}
        end,
        restoreState=function(state)
            if type(state)~="table" or state.epoch~=Epoch() or NS.IsCombatLocked() or type(state.values)~="table" then return false end
        ...
        movePosition=function(request)
            ...
            local values={[spec.xKey]=math.floor(x*10+0.5)/10,[spec.yKey]=math.floor(y*10+0.5)/10}
            for key,value in pairs(spec.moveValues or {}) do values[key]=value end
        ...
        resetPosition=function()
            ...
            for _,key in ipairs(spec.resetKeys or {}) do values[key]=rules[key].default end
            return S.SetMany(id,values)
        end,
        extraControls=spec.extraControls,
        openSettings=function() S.Open(id);return true end,
    }
end

-- spec: label, getFrame(), xKey, yKey, point (string or function), pointKey,
-- isEnabled(), order, extraControls, moveValues, resetKeys, place(x, y) (drag
-- preview for x/y that are not UIParent offsets). Returns true when MSUF Edit Mode accepted it.
function S.RegisterOwnedMover(id,elementID,spec)
    ...
    if ok then owners[id][elementID]=true;EnsureSession(api) end
    return ok
end
...
function S.OpenEditMode(id,elementID)
    ...
    local em=_G.MSUF_EM2
    if em and em.State and type(em.State.Enter)=="function" then em.State.Enter("player");return true end
    return false
end
...
function S.SetEditMode(active)
    ...
    S.editMode=active
    for id in pairs(owners) do if S.states[id].active then S.Apply(id) end end
end

function S.RefreshEditMover(id)
    if S.states[id] and S.states[id].active and S.instances[id] and S.instances[id].RegisterMovers then
        S.instances[id]:RegisterMovers()
    end
end
