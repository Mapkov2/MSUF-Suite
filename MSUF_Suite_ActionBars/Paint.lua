...
function AB.ResolveAPI()
    ...
    if api.durations and curves and not AB.desatCurve then
        ...
        if step then AB.desatCurve:SetType(step);AB.alphaCurve:SetType(step) end
        AB.desatCurve:AddPoint(0,0);AB.desatCurve:AddPoint(0.001,1)
    end
end
...
local function Tint(rec)
    ...
    rec.tint=key
    if key==4 then icon:SetVertexColor(style.rr,style.rg,style.rb)
    elseif key==1 then icon:SetVertexColor(1,1,1)
    elseif key==2 then icon:SetVertexColor(.5,.5,1)
    else icon:SetVertexColor(.4,.4,.4) end
end
...
local function ReleaseRange(rec)
    ...
    if count>0 then rangeRefs[slot]=count;return end
    rangeRefs[slot]=nil
...
local function AcquireRange(rec)
    ...
    if not M.config.rangeColoring or not rec.filled or not slot then ReleaseRange(rec);return end
...
local function Count(rec)
    ...
    if api.DisplayCount then count:SetText(api.DisplayCount(slot))
...
local function CooldownFeedback(rec,active,duration)
    ...
    if not desaturate and not alpha then
        if rec.feedback then rec.feedback=nil;button.icon:SetDesaturation(0);button:SetAlpha(1) end
        return
    end
    rec.feedback=true
    if not active then button.icon:SetDesaturation(0);button:SetAlpha(1);return end
    if api.durations then
        ...
        if alpha then button:SetAlpha(object:EvaluateRemainingDuration(AB.alphaCurve)) else button:SetAlpha(1) end
    else
...
local function Cooldown(rec)
    ...
    if loc and api.LoC and AB.locActive then
        ...
        if Public(shown) and shown then
            ...
            if api.durations and api.LoCDuration then loc:SetCooldownFromDurationObject(api.LoCDuration(slot))
            elseif Public(lossInfo.startTime) then loc:SetCooldown(lossInfo.startTime,lossInfo.duration,lossInfo.modRate) end
            rec.locCooldownShown=true
        else
            if rec.locCooldownShown then loc:Clear();rec.locCooldownShown=nil end
        end
    elseif loc and rec.locCooldownShown then
        loc:Clear();rec.locCooldownShown=nil
    end
    if charge and rec.noChargeEpoch~=chargeEpoch then
        ...
        if Public(recharging) and recharging and not replace then
            if api.durations and api.ChargeDuration then charge:SetCooldownFromDurationObject(api.ChargeDuration(slot))
            else charge:SetCooldown(charges.cooldownStartTime,charges.cooldownDuration,charges.chargeModRate) end
            rec.chargeCooldownShown=true
        elseif readable and Public(recharging) then
            if rec.chargeCooldownShown then charge:Clear();rec.chargeCooldownShown=nil end
        end
    ...
    elseif active and not replace then
        if api.durations then cooldown:SetCooldownFromDurationObject(api.CooldownDuration(slot))
        else cooldown:SetCooldown(info.startTime,info.duration,info.modRate) end
        rec.cooldownShown=true
    else
        if rec.cooldownShown then cooldown:Clear();rec.cooldownShown=nil end
    end
...
local function SetGlow(rec,show)
    ...
    if rec.glow then
        ...
        if rec.glowEdges then for i=1,4 do rec.glowEdges[i]:Hide() end end
    end
    ...
    if mode==1 and ActionButtonSpellAlertManager then
        ...
        if alert then local size=button:GetWidth();alert:SetSize(size*1.4,size*1.4) end
    elseif mode==2 then
...
local function GlowCheck(rec)
    ...
    elseif flyout and api.Overlayed and type(GetFlyoutInfo)=="function" and type(GetFlyoutSlotInfo)=="function" then
        ...
        for i=1,Public(slots) and type(slots)=="number" and slots or 0 do
            ...
            if Public(overlayed) and overlayed then show=true;break end
        end
    end
...
local function Clear(rec)
    ...
    if rec.feedback then rec.feedback=nil;if button.icon then button.icon:SetDesaturation(0) end;button:SetAlpha(1) end
    SetGlow(rec,false)
    ReleaseRange(rec)
end
...
local function Paint(rec)
    ...
    if not (Public(has) and has) then Clear(rec);return end
...
local function Refill(bar)
    ...
    for i=1,#bar.buttons do
        ...
        if rec.filled and i<=(bar.count or 12) then count=count+1;filled[count]=rec end
    end
    for i=count+1,#filled do filled[i]=nil end
end

function AB.Remap()
    for _,list in pairs(slotMap) do for i=#list,1,-1 do list[i]=nil end end
    for i=1,#AB.owned do
        ...
        if slot then
            ...
            if not list then list={};slotMap[slot]=list end
            list[#list+1]=rec
        end
    end
end
...
local function NativeCount(rec)
    ...
    if not count then return end
    if not M.config.hideEmptyCharges then count:SetAlpha(1);return end
...
local function NativeUsablePost(button,slot,usable,noMana)
    ...
    if rec.outOfRange then rec.tint=nil;Tint(rec) end
end
...
local function NativeColor(rec)
    ...
    AcquireRange(rec)
    if M.config.rangeColoring or wasRangeTint then rec.tint=nil;Usable(rec) end
end
...
local function RefreshNativeButton(rec)
    ...
    NativeState(rec)
    NativeFeedback(rec)
    if M.config.procGlow==2 then GlowCheck(rec) else SetGlow(rec,false) end
end
...
local function Capped(kind,now,fn)
    ...
    if wait>0 then Schedule(wait);return end
    dirty[kind]=false
    last[kind]=now
    Walk(fn)
...
Flush=function()
    ...
    if dirty.full then
        ...
        for index=1,AB.BAR_COUNT do local bar=AB.bars[index];if bar then dirtyBars[bar]=true end end
    ...
    for bar in pairs(dirtyBars) do
        dirtyBars[bar]=nil
        if Visible(bar) then AB.PaintBar(bar) else KeyTexts(bar) end
    end
    for slot in pairs(dirtySlots) do
        ...
        if list then
            for i=1,#list do
                local rec=list[i]
                if not rec.native and Visible(rec.bar) then Paint(rec);refillBars[rec.bar]=true end
            end
        end
    end
    for bar in pairs(refillBars) do refillBars[bar]=nil;Refill(bar) end
    Capped("cooldown",now,Cooldown)
    Capped("usable",now,Usable)
    if dirty.state then dirty.state=false;Walk(State);if not M.config.castHighlight then WalkNative(NativeState) end end
    if dirty.count then dirty.count=false;Walk(Count);if M.config.hideEmptyCharges then WalkNative(NativeCount) end end
    if dirty.icon then
        dirty.icon=false
        Walk(Icon)
        for index=1,10 do local bar=AB.bars[index];if bar and not bar.native and Visible(bar) then Refill(bar) end end
    end
    if nativeFull then
    ...
    if not NS.IsCombatLocked() then
        for index=1,10 do
            local bar=AB.bars[index]
            if bar and (gridDirty or gridBars[bar]) then gridBars[bar]=nil;AB.Execute(bar.header,[[self:ChildUpdate("grid")]]) end
        end
        gridDirty=false
        if routingDirty then routingDirty=false;AB.UpdateRouting() end
    end
end

local function Mark(kind) dirty[kind]=true;Schedule(0) end
function AB.MarkAll() dirty.full=true;routingDirty=true;gridDirty=true;Schedule(0) end
function AB.MarkBar(bar) dirtyBars[bar]=true;Schedule(0) end

------------------------------------------------------------------ events
local function SlotChanged(_,_,slot)
    if not Public(slot) or type(slot)~="number" or slot==0 then AB.MarkAll();return end
    ...
    if list then
        ...
        for i=1,#list do
            ...
            if not rec.native and rec.flyout~=flyout then rec.flyout=flyout;routingDirty=true end
        end
    end
    Schedule(0)
end
...
local function UsableChanged(_,_,changes)
    if type(changes)~="table" then Mark("usable");return end
    for i=1,#changes do
...
local function GlowMatch(rec)
    ...
    if id==glowSpell then SetGlow(rec,glowShow and M.config.procGlow~=3)
    elseif flyout then GlowCheck(rec) end
end
local function Glow(_,event,spell)
    if M.config.procGlow==3 then return end
    if not Public(spell) or type(spell)~="number" then AB.MarkAll();return end
...
local function Bindings()
    for index=1,AB.BAR_COUNT do local bar=AB.bars[index];if bar then KeyTexts(bar) end end
    AB.UpdateRouting()
end

local function CVarChanged(_,_,name)
    if name=="ActionButtonUseKeyDown" or name=="lockActionBars" or name=="LOCK_ACTIONBAR" then
        if NS.IsCombatLocked() then AB.attributesPending=true else AB.UpdateClickAttributes() end
    end
end
...
local function FormsChanged()
    ...
    if NS.IsCombatLocked() then S.Queue("actionbars") else M:Refresh() end
end

local function RegenEnabled()
    if AB.routingPending then AB.UpdateRouting() end
    if AB.attributesPending then AB.attributesPending=nil;AB.UpdateClickAttributes() end
...
local EVENTS={
    ...
    UPDATE_SHAPESHIFT_FORM=function() chargeEpoch=chargeEpoch+1;Mark("icon") end,
    SPELL_UPDATE_ICON=function() chargeEpoch=chargeEpoch+1;Mark("icon") end,
    SPELLS_CHANGED=function() chargeEpoch=chargeEpoch+1;Mark("icon") end,
    UPDATE_SUMMONPETS_ACTION=function() chargeEpoch=chargeEpoch+1;Mark("icon") end,
    BAG_UPDATE_DELAYED=function() Mark("count") end,
    PLAYER_EQUIPMENT_CHANGED=function() chargeEpoch=chargeEpoch+1;AB.MarkAll() end,
    PLAYER_ENTERING_WORLD=function() chargeEpoch=chargeEpoch+1;AB.MarkAll() end,
    ...
    UPDATE_SHAPESHIFT_FORMS=function(...) chargeEpoch=chargeEpoch+1;FormsChanged(...) end,
...
function AB.SyncOptionalEvents()
    ...
    for event in pairs(optionalEvents) do
        ...
        if wanted then context:Event(event,EVENTS[event],true)
...
function AB.ShowTooltip(rec)
    ...
    if GameTooltip_SetDefaultAnchor then GameTooltip_SetDefaultAnchor(tip,rec.button) else tip:SetOwner(rec.button,"ANCHOR_RIGHT") end
...
function AB.StopDispatcher()
    ...
    for i=1,#AB.owned do
        local rec=AB.owned[i]
        if not rec.native then ReleaseRange(rec);SetGlow(rec,false) end
    end
    for kind in pairs(dirty) do dirty[kind]=false end
    for slot in pairs(dirtySlots) do dirtySlots[slot]=nil end
