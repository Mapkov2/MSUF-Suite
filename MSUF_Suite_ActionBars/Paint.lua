...
local function Count(rec)
    ...
    local alpha=1
    if M.config.hideEmptyCharges and api.Charges then
        local charges=api.Charges(slot)
        local maximum,current=charges and charges.maxCharges,charges and charges.currentCharges
...
local function Cooldown(rec)
    ...
    local info=api.Cooldown and api.Cooldown(slot)
    ...
    local recharging=charges and charges.isActive
    if charge then
    ...
    if active and not replace then
    ...
    CooldownFeedback(rec,active and not replace,info and info.duration)
end
...
local function KeyTexts(bar)
    for i=1,#bar.buttons do
        local rec=bar.buttons[i]
        local text=rec.owned and rec.button.HotKey or rec.keyText
...
Flush=function()
    ...
    for slot in pairs(dirtySlots) do
        ...
        if list then
            for i=1,#list do
                local rec=list[i]
                if Visible(rec.bar) then Paint(rec);refillBars[rec.bar]=true end
            end
        end
    end
    ...
    if dirty.state then dirty.state=false;Walk(State) end
    if dirty.count then dirty.count=false;Walk(Count) end
    if dirty.icon then
        dirty.icon=false
        Walk(Icon)
        for index=1,10 do local bar=AB.bars[index];if bar and Visible(bar) then Refill(bar) end end
    end
...
local function SlotChanged(_,_,slot)
    ...
    if list then
        ...
        for i=1,#list do
            ...
            if rec.flyout~=flyout then rec.flyout=flyout;routingDirty=true end
        end
    end
    Schedule(0)
end
...
local function UsableChanged(_,_,changes)
    ...
    for i=1,#changes do
        ...
        if list then
            for n=1,#list do
                local rec=list[n]
                if rec.filled and Visible(rec.bar) then Usable(rec,change.usable,change.noMana) end
            end
        end
    end
end
...
local function RangeChanged(_,_,slot,inRange,checksRange)
    ...
    for i=1,#list do
        local rec=list[i]
        if rec.rangeSlot==slot then rec.outOfRange=out;Tint(rec) end
    end
end
...
local function LossOfControl() AB.locActive=true;Mark("cooldown") end
...
local EVENTS={
    ...
    SPELL_UPDATE_COOLDOWN=function() Mark("cooldown") end,
    SPELL_UPDATE_CHARGES=function() Mark("cooldown");Mark("count") end,
    LOSS_OF_CONTROL_ADDED=LossOfControl,
    LOSS_OF_CONTROL_UPDATE=LossOfControl,
    ACTIONBAR_UPDATE_USABLE=function() Mark("usable") end,
    SPELL_UPDATE_USABLE=function() Mark("usable") end,
    ...
    UPDATE_SHAPESHIFT_FORM=function() Mark("icon") end,
    SPELL_UPDATE_ICON=function() Mark("icon") end,
    SPELLS_CHANGED=function() Mark("icon") end,
    UPDATE_SUMMONPETS_ACTION=function() Mark("icon") end,
    BAG_UPDATE_DELAYED=function() Mark("count") end,
    PLAYER_EQUIPMENT_CHANGED=function() AB.MarkAll() end,
    PLAYER_ENTERING_WORLD=function() AB.MarkAll() end,
    ...
    UPDATE_SHAPESHIFT_FORMS=FormsChanged,
...
local function Shown(header)
    ...
    if bar then AB.MarkBar(bar) end
end
...
function AB.StartDispatcher()
    ...
    for i=1,#AB.owned do
        ...
        if cooldown and not AB.cooldownOwner[cooldown] then
