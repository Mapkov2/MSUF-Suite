...
local function Reassert()
    ...
    if pager then Unmouse(pager.UpButton);Unmouse(pager.DownButton) end
...
function AB.ReadBlizzard()
    ...
    for index=1,12 do
        local frame
        if index<=8 then frame=AB.Frame(AB.NATIVE_BARS[index])
        elseif index==11 then frame=AB.Frame("StanceBar")
        elseif index==12 then frame=AB.Frame("PetActionBar") end
...
function AB.Dispose()
    ...
    if art and art~=main and not art:IsProtected() then
        art:SetAlpha(0)
        Unmouse(AB.Frame("MainMenuBarMaxLevelBar"));Unmouse(AB.Frame("MainMenuBarPerformanceBarFrameButton"))
    end
    return true
end
...
function AB.ReparentLeaveButton()
    ...
    if leave:GetParent()~=main then AB.leaveMoved=true;return end
...
function AB.Adopt(index)
    ...
    for i=1,10 do
        ...
        if rec then
            ...
            if index==12 then control:SetAttribute("grid"..i,i<=count and showEmpty) else control:SetAttribute("grid"..i,nil) end
        else
