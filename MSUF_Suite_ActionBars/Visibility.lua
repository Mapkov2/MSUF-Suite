...
local function SetHover(bar,on)
    ...
    bar.hover=on
    if M.config.mouseoverShowAll then AB.UpdateAllAlpha() else AB.UpdateAlpha(bar) end
end
...
function AB.ApplyDrag()
    ...
    AB.UpdateAllAlpha()
    if NS.IsCombatLocked() then AB.dragPending=true;return end
    AB.dragPending=nil
...
function AB.StopVisibility()
    for index=1,AB.BAR_COUNT do
        local bar=AB.bars[index]
        if bar then
            if bar.visDriver then UnregisterStateDriver(bar.header,"vis");bar.visDriver=nil end
            SetForced(bar,"forceshow",nil);SetForced(bar,"dragshow",nil)
